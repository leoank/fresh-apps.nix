...
Each package and flake input gets one entry. Packages with no ``version``
attribute, or with ``passthru.hideFromDocs`` and no
``passthru.updateEvenIfHidden`` opt-in, are skipped.
"""
...
def discover_packages(
    ...
    """Discover packages with a ``version`` attribute via a single ``nix eval``."""
...
def write_matrix(matrix_items: list[MatrixItem]) -> None:
    """Write the matrix and ``has-updates`` flag to GITHUB_OUTPUT."""
