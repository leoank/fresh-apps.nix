...
def gh_get_pr_number(branch: str) -> str | None:
    """Return the PR number for ``branch`` or None if no PR exists."""
    result = run(
        [
            "gh",
            "pr",
...
def build_config(
    *,
    ...
    """Compose branch, title, body, and commit message for an update PR."""
    match update_type:
...
def create_or_update_pr(config: PrConfig, *, labels: str, auto_merge: bool) -> None:
    """Commit, push, and create or update the PR."""
    run(["git", "add", "."])
